...
SevSeg display; //Instantiate a seven segment controller object

char text[11][2] = {"C", "H", "I", "C", "C", "O", "D", "B", "0", "0","-"};

void setup()
{
  byte numDigits = 1;   
  byte digitPins[] = {2}; //Digits: 1,2,3,4 <--put one resistor (ex: 220 Ohms, or 330 Ohms, etc, on each digit pin)
  ...
  display.setBrightness(90); //Note: 100 brightness simply corresponds to a delay of 2000us after lighting each segment. A brightness of 0 
...
void loop()
{
  display.setChars(text[millis()/500%11]);
  display.refreshDisplay(); // Must run repeatedly; don't use blocking code (ex: delay()) in the loop() function or this won't work right
}
